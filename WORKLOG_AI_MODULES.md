...
- Added engine token step via `GET token` and extraction of dynamic `Ai_token`.
- Changed `release_task` and `query_result` calls to `application/x-www-form-urlencoded` with:
  - `Ai_token=<jwt>`
  - `task_id_list=["<uuid>"]`
  - `app=studio-web`
