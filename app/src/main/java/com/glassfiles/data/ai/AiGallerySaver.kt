 ...
 * Photos / Gallery alongside their other pictures and videos.
 *
 * On API 29+ we use scoped storage via `MediaStore` (`RELATIVE_PATH`) and let
 * the system place the file under `Pictures/GlassFiles_AI/` (or
 * `Movies/GlassFiles_AI/` for video). On API 28 (this app's `targetSdk`) we
 * fall back to the legacy file path under the public Pictures / Movies dir
